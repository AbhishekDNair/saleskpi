...
function setKpiChange(elId, pct, isGood = true) {
  ...
  el.innerHTML = `${positive ? '↑' : '↓'} ${positive ? '+' : ''}${pct.toFixed(1)}%`;
}

function setTrendIndicator(elId, pct, isPositiveGood = true) {
  const el = document.getElementById(elId);
  if (!el) return;
  const isUp = pct >= 0;
  const isGood = isPositiveGood ? isUp : !isUp;
  el.className = 'trend-indicator ' + (isGood ? 'up' : 'down');
  el.innerHTML = `${isUp ? '↑' : '↓'} ${isUp ? '+' : ''}${pct.toFixed(1)}%`;
}
...
function updateKPICards(filtered, prevFiltered) {
  ...
  setTrendIndicator('mt-lost', -18.5, false);
  ...
  setTrendIndicator('mt-repeat', 4.2, true);
