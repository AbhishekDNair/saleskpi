...
function animateNumber(el, target, prefix = '', suffix = '', decimals = 0) {
  ...
  function step(now) {
    ...
    else { el.textContent = prefix + target.toFixed(decimals) + suffix; el.setAttribute('data-val', target); }
  }
...
function renderTable(filtered) {
  ...
  document.getElementById('tableCount').textContent = `Showing ${start + 1}–${Math.min(start + TABLE_PAGE_SIZE, total)} of ${total.toLocaleString()} records`;
